 ...
 */
    ...
    //Creating an object of Class Dog
        Dog d1 = new Dog();
        // In this d1 is reference variable which stores the address of object of Dog
        // d1 is stored in stack memory while the object is stored in heap memory , d1 stores the address of object which is stored in heap memory
//        new is keyword which dynamically allocate the memory to class object at runtime
        // Dog() is the constructor which is invoked when a object is created and perform actions

        Dog d4 = d1;   // When we create this then d2 points the same memory location which is pointed by d1
        ...
        */
        ...
        Static blocks, also known as static initialization blocks, are used to initialize static variables. They are executed only once when the class is first loaded into memory,
        before the main method or any other static methods. Static blocks can be used to perform complex static initializations that cannot be done in a single line of code.
...
Static Nested Classes: Useful for logically grouping helper classes, builder classes, or utility classes that are only relevant to the outer class. They help keep the code organized and maintainable.
        ...
        3.If a class doesn't have any constructor, the java compiler automatically creates a default constructor during run time.
...
class Dog{   // Must try(not compulsory) to use the first letter of name of the class CAPITAL
    ...
    String name;    // These have default access modifier (package - private)
    int age;
    String color;
    ...
    @Override
    protected void finalize() throws Throwable {    // It works when the memory is freed by the Garbage Collector(which is a automatic process)
        super.finalize();
